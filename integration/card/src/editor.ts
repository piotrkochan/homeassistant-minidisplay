...
export class MiniDisplayEditor extends LitElement {
  ...
  private transitionEditor(dashboard: Dashboard) {
    const transition = dashboard.transition ?? { type: "none" as const };
    const set = (next: PageTransition) => { dashboard.transition = next; this.changed(); };
