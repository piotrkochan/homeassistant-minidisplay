...
export class MiniDisplayEditor extends LitElement {
  ...
  private closeActionMenusOnOutsideClick = (event: PointerEvent) => {
    ...
    this.renderRoot.querySelectorAll<HTMLDetailsElement>("details.menu[open]").forEach((menu) => {
  ...
  private checkbox(label: string, value: boolean, update: (value: boolean) => void, disabled = false, title = "") {
    return html`<label class="check" title=${title}><input type="checkbox" .checked=${value} ?disabled=${disabled} @change=${(event: Event) => update((event.target as HTMLInputElement).checked)}>${label}</label>`;
  }
  ...
  private textPosition(label: string, style: Style, defaultHorizontal: NonNullable<Style["horizontalAlign"]> = "center", defaultVertical: NonNullable<Style["verticalAlign"]> = "middle") {
    ...
    const selected = positions.find((position) => position.horizontal === horizontal && position.vertical === vertical)!;
    return html`<details class="position-field"><summary>${label} · ${selected.label}</summary><div class="position-grid" role="radiogroup" aria-label=${label}>${positions.map((position) => {
  ...
  private cardSettings(card: DisplayCard, rowIndex: number, cardIndex: number) {
    ...
    return html`<section class="card-settings"><div class="card-head"><div class="card-title"><strong>${card.title?.trim() || html`<em>Unnamed card</em>`}</strong>${card.visibility ? html`<span class="condition-mark"><ha-icon icon="mdi:eye-settings-outline"></ha-icon>Conditional</span>` : nothing}</div>${this.menu(html`<button @click=${() => this.openVisibility("card", rowIndex, cardIndex)}>Visibility</button><button @click=${() => { cards.splice(cardIndex + 1, 0, structuredClone(card)); this.selected = { row: rowIndex, card: cardIndex + 1 }; this.changed(); }}>Duplicate</button><button class="danger" ?disabled=${cards.length === 1} @click=${() => { if (cards.length > 1) { cards.splice(cardIndex, 1); this.selected = undefined; this.changed(); } }}>Delete</button>`)}</div><div class="grid">${this.select("Type", card.type, ["number", "text", "clock", "status"], (input) => { Object.keys(card).forEach((key) => delete (card as unknown as Record<string, unknown>)[key]); Object.assign(card, newCard(input as DisplayCard["type"])); this.changed(); })}${this.field("Title", card.title, (input) => { card.title = input; this.changed(); })}<p class="hint">${hints[card.type]}</p>${["number", "status", "text"].includes(card.type) ? this.entity(card) : nothing}${card.type === "number" ? html`${this.field("Unit", card.unit, (input) => { card.unit = input; this.changed(); })}${this.select("Progress", card.progress ?? "none", ["none", "bar", "ring"], (input) => { card.progress = input as DisplayCard["progress"]; this.changed(); })}${card.progress && card.progress !== "none" ? html`${this.field("Minimum", card.minimum, (input) => { card.minimum = Number(input); this.changed(); }, "number")}${this.field("Maximum", card.maximum, (input) => { card.maximum = Number(input); this.changed(); }, "number")}` : nothing}` : nothing}${card.type === "text" ? this.field("Static text", card.text, (input) => { card.text = input; this.changed(); }) : nothing}${card.type === "status" ? html`${this.field("On text", card.onText, (input) => { card.onText = input; this.changed(); })}${this.field("Off text", card.offText, (input) => { card.offText = input; this.changed(); })}` : nothing}</div>${this.valueMappingsEditor(card)}${this.colorMappingsEditor(card)}${this.styleEditor(card)}</section>`;
  }

  private rowEditor(row: DisplayRow, rowIndex: number) {
    const page = this.dashboard!.pages[this.pageIndex];
    return html`<section class="row-panel"><div class="row-head"><div class="row-title"><strong>Row ${rowIndex + 1}</strong><small>${row.cards.length} ${row.cards.length === 1 ? "card" : "cards"}</small>${row.visibility ? html`<span class="condition-mark"><ha-icon icon="mdi:eye-settings-outline"></ha-icon>Conditional</span>` : nothing}</div>${this.menu(html`<button @click=${() => this.openVisibility("row", rowIndex)}>Visibility</button><button @click=${() => { page.rows.splice(rowIndex + 1, 0, structuredClone(row)); this.changed(); }}>Duplicate</button><button class="danger" ?disabled=${page.rows.length === 1} @click=${() => { if (page.rows.length > 1) this.requestDeleteRow(rowIndex); }}>Delete</button>`)}</div>${this.field("Row title", row.title, (input) => { row.title = input; this.changed(); })}<nav class="card-tabs" aria-label=${`Cards in row ${rowIndex + 1}`}>${row.cards.map((card, cardIndex) => { const active = this.selected?.row === rowIndex && this.selected?.card === cardIndex; return html`<button draggable="true" class="tab ${active ? "active" : ""} ${this.draggedCard?.row === rowIndex && this.draggedCard.index === cardIndex ? "dragging" : ""}" aria-label=${card.title?.trim() || `Unnamed card ${cardIndex + 1}`} aria-expanded=${active} @dragstart=${(event: DragEvent) => this.dragCard(rowIndex, cardIndex, event)} @dragover=${(event: DragEvent) => event.preventDefault()} @drop=${(event: DragEvent) => this.dropCard(rowIndex, cardIndex, event)} @dragend=${() => { this.draggedCard = undefined; this.requestUpdate(); }} @click=${() => this.selected = active ? undefined : { row: rowIndex, card: cardIndex }}>${card.title?.trim() || html`<em>Unnamed card</em>`}</button>`; })}${row.cards.length < 3 ? html`<button class="icon-button" title="Add card" aria-label="Add card" @click=${() => { row.cards.push(newCard()); this.selected = { row: rowIndex, card: row.cards.length - 1 }; this.changed(); }}><ha-icon icon="mdi:plus"></ha-icon></button>` : nothing}</nav>${this.selected?.row === rowIndex ? this.cardSettings(row.cards[this.selected.card], rowIndex, this.selected.card) : nothing}</section>`;
  }
  ...
  private renderEditor() {
    ...
    const enabledPages = dashboard?.pages.filter((item) => item.enabled !== false).length ?? 0;
      ...
      <ha-card class="editor-card">
        <div class="editor-heading">
          ...
          <div class="save-area">
            ...
            <div class="save-actions"><ha-button .disabled=${!dirty} @click=${this.discard}>Discard</ha-button><ha-button .disabled=${!dirty || this.syncState === "syncing"} @click=${() => void this.save()}>Save</ha-button></div>
          </div>
        </div>
        ${page && dashboard ? html`
          <div class="editor-content">
            <nav class="tabs" aria-label="Dashboard pages">
              ${dashboard.pages.map((item, index) => html`
                <button class="tab ${index === this.pageIndex ? "active" : ""} ${item.enabled === false ? "inactive" : ""}" aria-pressed=${index === this.pageIndex} @click=${() => void this.showPage(index)}>
            ...
            <details class="page-settings">
              <summary class="page-summary"><span class="page-summary-copy"><span>Page settings</span><small>${page.durationSeconds ?? 10}s · ${page.enabled === false ? "Disabled" : "Enabled"}${page.showTitle === false ? " · title hidden" : ""}</small></span><button class="icon-button danger" aria-label="Delete page" title="Delete page" ?disabled=${dashboard.pages.length <= 1} @click=${(event: Event) => { event.preventDefault(); event.stopPropagation(); this.deletePage(); }}><ha-icon icon="mdi:delete-outline"></ha-icon></button></summary>
              <div class="page-settings-grid">
                ${this.field("Page title", page.title, (input) => { page.title = input; this.changed(); })}
                ${this.field("Duration (seconds)", page.durationSeconds, (input) => { page.durationSeconds = Number(input); this.changed(); }, "number")}
                <div class="page-options">
                  ${this.checkbox("Enabled", page.enabled !== false, (input) => { page.enabled = input; this.changed(); }, page.enabled !== false && enabledPages <= 1, "At least one page must stay enabled")}
                  ${this.checkbox("Show title", page.showTitle !== false, (input) => { page.showTitle = input; this.changed(); })}
                </div>
                ${page.showTitle !== false ? html`<div class="page-title-position">${this.segmented("Title position", page.titlePosition ?? "top", [
                  { value: "top" as const, label: "Top", icon: "mdi:format-vertical-align-top" },
                  { value: "right" as const, label: "Right", icon: "mdi:format-horizontal-align-right" },
                  { value: "bottom" as const, label: "Bottom", icon: "mdi:format-vertical-align-bottom" },
                  { value: "left" as const, label: "Left", icon: "mdi:format-horizontal-align-left" },
                ], (input) => { page.titlePosition = input; this.changed(); })}</div>` : nothing}
                <details class="page-appearance">
                  <summary>Page appearance</summary>
                  <div class="page-appearance-grid">
                    <mini-display-color-field label="Page background" .value=${pageStyle.background ?? ""} @color-changed=${(event: CustomEvent<string>) => { page.style = { ...(page.style ?? {}), background: event.detail || undefined }; this.changed(); }}></mini-display-color-field>
                    ${page.showTitle !== false ? html`
                      <mini-display-color-field label="Title background" .value=${pageTitleStyle.background ?? ""} @color-changed=${(event: CustomEvent<string>) => { page.titleStyle = { ...(page.titleStyle ?? {}), background: event.detail || undefined }; this.changed(); }}></mini-display-color-field>
                      <mini-display-color-field label="Title text" .value=${pageTitleStyle.foreground ?? ""} @color-changed=${(event: CustomEvent<string>) => { page.titleStyle = { ...(page.titleStyle ?? {}), foreground: event.detail || undefined }; this.changed(); }}></mini-display-color-field>
                      ${this.select("Title font size", pageTitleStyle.fontSize ?? "small", ["small", "medium", "large", "xlarge"], (input) => { page.titleStyle = { ...(page.titleStyle ?? {}), fontSize: input as Style["fontSize"] }; this.changed(); })}
                    ` : nothing}
                  </div>
                </details>
                <details class="advanced-settings">
                  ...
                  <div class="advanced-settings-content">
                    ${this.field("Page ID", page.id, (input) => { page.id = input; this.changed(); })}
                  </div>
                </details>
              </div>
            </details>
            ${this.transitionEditor(page)}
            <div class="rows">${page.rows.map((row, index) => this.rowEditor(row, index))}</div>
            ${page.rows.length < 6 ? html`<button class="add-button" @click=${() => { page.rows.push(newRow()); this.changed(); }}>Add row</button>` : nothing}
          </div>
        ` : html`<div class="loading"><p>No layout configured for this display.</p><ha-button @click=${this.createLayout}>Create layout</ha-button></div>`}
      </ha-card>
    `;
  }
