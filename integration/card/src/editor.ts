        ...
        ${page && dashboard ? html`
          <div class="editor-content">
            ...
            <details class="page-settings">
              <summary class="page-summary"><span>Page settings</span><button class="icon-button danger" aria-label="Delete page" title="Delete page" ?disabled=${dashboard.pages.length <= 1} @click=${(event: Event) => { event.preventDefault(); event.stopPropagation(); this.deletePage(); }}><ha-icon icon="mdi:delete-outline"></ha-icon></button></summary>
              <div class="page-settings-grid">
                ${this.field("Title", page.title, (input) => { page.title = input; this.changed(); })}
                ${this.field("Page ID", page.id, (input) => { page.id = input; this.changed(); })}
                ${this.field("Duration in seconds", page.durationSeconds, (input) => { page.durationSeconds = Number(input); this.changed(); }, "number")}
