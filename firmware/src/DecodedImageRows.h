...
class DecodedImageRows {
 public:
  bool copy(const char *id, uint16_t row, uint16_t width, uint16_t *out) const {
    for (const auto &entry : entries_) {
      if (entry.valid && entry.row == row && entry.width == width &&
  ...
  void store(const char *id, uint16_t row, uint16_t width, const uint16_t *pixels) {
